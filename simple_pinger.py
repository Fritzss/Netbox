...
# connect to Netbox
nb = pynetbox.api('https://<your netbox url>:443', token='<netbox token>')
...
def disableIp(addr):
    ...
    try:
        ip_update = nb.ipam.ip_addresses.get(address=addr)
        if ip_update != None and str(ip_update.status).lower() != 'deprecated' and  str(ip_update.status).lower() != 'reserved':
